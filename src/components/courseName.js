...
import { save, loaderCall, getBooksFromDb } from '../store/action/action';
...
import IMGS from "../assets/images/folder.png"
...
class CourseName extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        let obj = nextProps.folderName
        this.setState({
            data: obj,
            forSearchCalcled: obj
        })
    }
    ...
    search() {
        ...
        let cloneData = this.props.folderName
        let sortedData = []
        for (var i = 0; i < cloneData.length; i++) {
            if (cloneData[i].slice(0, cloneSearchData.length).toLowerCase() === cloneSearchData.toLowerCase()) {
            ...
            <div>
                ...
                >
                    ...
                    <div className="form-inline">
                        <div >
                            <input value={this.state.search} onChange={this._onChangeVlaue('search')} className="form-control mr-sm-2" type="search" placeholder="Search Course Name" aria-label="Search Book Name" />
                            {
                                (this.state.search !== "") ? (<span className="hoverCross" onClick={this.clearSearch.bind(this)} style={{ marginLeft: "-12%", color: "grey", fontWeight: "bold", }}>x</span>
                                ) : null
                            }
                        </div>
...

                    {
                        (this.props.isLoader === true) ?
                            ...
                            : (
                                ...
                                <table className="table table-striped listHeading" style={{ width: "40%" }}>
                                    <thead >
                                        <tr>
                                            <th>COURSE NAME AND BATCH NUMBER</th>
                                        </tr>
                                    </thead>
                                    <tbody
                                    ...
                                    >
                                        {
                                            ...
                                            this.state.data.map((data, index) => {
                                                return (
                                                    ...
                                                    <tr key={index}>
                                                        {/* <td>{data.courseName + " " + data.batchNumber}</td> */}
                                                        <td className="tdHover"
                                                        // style={{ fontWeight:"bold" }}
                                                        >


                                                            {/* <Link to="ViewData" params={{ testvalue: "hello" }}>{data}</Link> */}
                                                            ...
                                                            >
                                                                ...
                                                                <div>
                                                                    <img style={{ marginRight: 30, marginLeft: 20 }} width={30} src={IMGS} alt="boohoo" className="img-responsive" />
...
export default connect(mapStateToProp, mapDispatchToProp)(CourseName);
